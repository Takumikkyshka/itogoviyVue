...
<template>
  ...
  <div class="lg:hidden">
    <div class="mx-4">
      <h1 class="pt-10 text-2xl">Shopping Cart</h1>
      <div
        class="w-full flex py-10 border-b border-[#A3A3A3]"
        v-for="product in cartStore.cartItems"
        :key="product.product.id"
      >
        <img
          class="w-22 object-contain"
          :src="`http://localhost:1452/${product.product.images[0]}`"
        />
        <div class="flex-1">
          <p>{{ product.product.brand }} || {{ product.product.name }}</p>
          ...
          <div class="flex items-center justify-between mt-4">
            <div class="flex items-center gap-2 ml-3">
              ...
              <p class="border border-[#D9D9D9] px-3 rounded-md py-1">{{ product.quantity }}</p>
            ...
            <button @click="cartStore.deleteFromCart(product.product.id)">
              <img src="/Close.svg" />
            </button>
          </div>
        </div>
