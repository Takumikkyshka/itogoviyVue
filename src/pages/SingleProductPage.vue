...
import ProductCard from '@/components/IndexPage/ProductCard.vue'
...
<template>
  <div v-if="product" class="hidden lg:block">
  ...
  <div v-if="product" class="lg:hidden">
    <div class="flex flex-col mx-4">
      <div class="py-10">
        <img class="w-full object-contain" :src="`http://localhost:1452/${product.images[0]}`" />
      </div>
      <div class="grid grid-cols-4 gap-4 pb-10">
        <img class="w-full object-contain" :src="`http://localhost:1452/${product.images[0]}`" />
        <img
          class="opacity-50 w-full object-contain"
          :src="`http://localhost:1452/${product.images[0]}`"
        />
        <img
          class="opacity-50 w-full object-contain"
      ...
      <div class="flex flex-col gap-6">
        <h1 class="text-[40px]">{{ product.brand }} || {{ product.name }}</h1>
        <div class="flex gap-4 items-center">
          <p class="text-[32px]">${{ product.discount_price ?? product.price }}</p>
          <p class="text-[#A0A0A0] text-2xl line-through" v-if="product.discount_price">
            ${{ product.price }}
          </p>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div
            class="bg-[#F4F4F4] text-center px-4 py-4 rounded-md flex flex-col items-center justify-center"
            v-for="characteristic in product?.characteristics"
            :key="characteristic.characteristic"
          >
            <p class="text-[#A7A7A7]">{{ characteristic.characteristic }}</p>
        ...
        <button
          class="w-full border py-4 rounded-md italic cursor-pointer"
          @click="toggleFavorite(product.id)"
        >
          {{ isFavorite(product.id) ? 'Delete from favorite' : 'Add to favorite' }}
        </button>
        <button
          @click="(cartStore.addToCart(product), router.push('/cart'))"
          class="w-full border py-4 rounded-md italic text-white bg-black cursor-pointer"
        >
          Add to Cart
        </button>

        <div class="flex gap-8 mt-2 justify-center text-center">
        ...
        <div>
          <h1 class="mt-12 mb-8 text-2xl">Details</h1>
          <p>
            Just as a book is judged by its cover, the first thing you notice when you pick up a
          ...
          <div>
            ...
            <div class="bg-[#FAFAFA] flex items-center justify-center gap-10 mt-20 mb-5 rounded-xl">
              <div class="">
                <p class="text-[56px] text-center">{{ product.rating }}</p>
                <p class="opacity-30">of {{ product.count_review }} reviwes</p>
              </div>
